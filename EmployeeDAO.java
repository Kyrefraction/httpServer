...
public class EmployeeDAO {
	...
	public ArrayList<Employee> selectEmployeeByName(String name) throws ClassNotFoundException, SQLException {
		Connection con = null;
		Statement st = null;
		ResultSet rs = null;
		Class.forName("org.sqlite.JDBC");
		...
		try {
			...
			con = DriverManager.getConnection(dbURL);
			st = con.createStatement();
			rs = st.executeQuery("SELECT * FROM employees WHERE Name LIKE '" + name + "';");
			while (rs.next()) {
				Employee E = new Employee(  rs.getString("ID"), rs.getString("Name"), rs.getString("Gender"),
						rs.getString("DOB"), rs.getString("Address"), rs.getString("Postcode"), rs.getString("NIN"),
						rs.getString("jobTitle"), rs.getString("StartDate"), rs.getString("Salary"),
	...
	public ArrayList<Employee> selectEmployeeByID(int currentID) throws ClassNotFoundException, SQLException {
		Connection con = null;
		Statement st = null;
		ResultSet rs = null;
		Class.forName("org.sqlite.JDBC");
		...
		try {
			...
			con = DriverManager.getConnection(dbURL);
			st = con.createStatement();
			rs = st.executeQuery("SELECT * FROM employees WHERE ID LIKE '" + currentID + "';");
			while (rs.next()) {
				Employee E = new Employee(  rs.getString("ID"), rs.getString("Name"), rs.getString("Gender"),
						rs.getString("DOB"), rs.getString("Address"), rs.getString("Postcode"), rs.getString("NIN"),
						rs.getString("jobTitle"), rs.getString("StartDate"), rs.getString("Salary"),
	...
	public void insertEmployeeAtId(Employee employee, int identified)
		...
		try {
			...
			String sql = "INSERT INTO employees (ID, Name, Gender, DOB, Address, Postcode, NIN, jobTitle, StartDate, Salary, Email)"
					+ " VALUES (" + identified + breaker + employee.getName() + breaker + employee.getGender() + breaker
					+ employee.getDob() + breaker + employee.getAddress() + breaker + employee.getPostcode() + breaker
					+ employee.getNatInscNo() + breaker + employee.getTitle() + breaker + employee.getStartDate() + breaker
					+ employee.getSalary() + breaker + employee.getEmail() + "');";
